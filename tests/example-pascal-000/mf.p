{4:}{9:}{$C-,A+,D-}{[$C+,D+]}{:9}program MF;label{6:}1,9998,9999;
{:6}const{11:}memmax=30000;maxinternal=100;bufsize=500;errorline=72;
halferrorline=42;maxprintline=79;screenwidth=768;screendepth=1024;
stacksize=30;maxstrings=2000;stringvacancies=8000;poolsize=32000;
movesize=5000;maxwiggle=300;gfbufsize=800;filenamesize=40;
poolname='MFbases:MF.POOL                         ';pathsize=300;
...
termout:alphafile;
{:31}{38:}strpool:packed array[poolpointer]of packedASCIIcode;
...
useerrhelp:boolean;errhelp:strnumber;{:74}{91:}interrupt:integer;
OKtointerrupt:boolean;{:91}{97:}aritherror:boolean;
{:97}{129:}twotothe:array[0..30]of integer;
...
{:628}{631:}inopen:0..6;openparens:0..6;
...
{:1203}procedure initialize;var{19:}i:integer;{:19}{130:}k:integer;
...
for k:=127 to 255 do charclass[k]:=20;{:199}{202:}hash[1].lh:=0;
...
gfoffset:=0;{:1153}{1184:}baseident:=0;{:1184}end;
{57:}procedure println;begin case selector of 3:begin writeln(termout);
writeln(logfile);termoffset:=0;fileoffset:=0;end;
2:begin writeln(logfile);fileoffset:=0;end;1:begin writeln(termout);
termoffset:=0;end;0,4,5:;end;end;
...
others:print(347)end;end;{:187}{195:}procedure begindiagnostic;
...
begin;print(263);terminput;end;if last=first then goto 10;
...
69:if fileptr>0 then begin printnl(264);
slowprint(inputstack[fileptr].namefield);print(265);printint(line);
...
case c of 81:begin print(273);selector:=selector-1;end;82:print(274);
83:print(275);end;print(276);println;break(termout);goto 10;end{:81};
...
begin if interaction=3 then interaction:=2;if logopened then error;
{if interaction>0 then debughelp;}history:=3;jumpout;end;end;
{:90}{:4}{26:}function aopenin(var f:alphafile):boolean;
...
{:65}{66:}procedure terminput;var k:0..bufsize;begin break(termout);
if not inputln(termin,true)then fatalerror(260);termoffset:=0;
selector:=selector-1;
...
overflow(316,memmax+1);40:mem[r].hh.rh:=0;{varused:=varused+s;}
getnode:=r;10:end;{:167}{172:}procedure freenode(p:halfword;s:halfword);
...
98:print(406);99:print(407);100:print(408);others:print(409)end;end;
{:189}{194:}procedure fixdateandtime;begin internal[17]:=12*60*65536;
internal[16]:=4*65536;internal[15]:=7*65536;internal[14]:=1776*65536;
end;{:194}{205:}function idlookup(j,l:integer):halfword;label 40;
...
tokenrecycle;end;others:confusion(491)end;freenode(q,2);end;end;end;
...
2:printchar(64);3:print(664);others:print(662)end;
...
{:1109}{1180:}29:if m=16 then print(1059)else print(1058);
{:1180}others:print(602)end;end;{:625}procedure showmacro(p:halfword;
...
if(q>1)or(r<>30000)then begin printnl(533);printint(n+yoff);
printchar(58);while q>1 do begin printweight(q,xoff);q:=mem[q].hh.rh;
end;print(534);while r<>30000 do begin printweight(r,xoff);
...
8:printpath(v,763,false);9:printpath(v,285,false);11:begin curedges:=v;
...
begin if t=17 then threshold:=2685 else threshold:=8;r:=29999;
...
if t1=17 then threshold:=1342 else threshold:=4;r:=29999;
...
8,9:tossknotlist(curexp);11:tossedges(curexp);others:end;curtype:=16;
...
while p<>30000 do begin mem[p].hh.lh:=mem[p].hh.lh-delta;
...
repeat q:=getnode(2);mem[q+1].hh.rh:=30000;mem[q+1].hh.lh:=1;
...
repeat{339:}q:=mem[p+1].hh.rh;r:=30000;
while q<>30000 do begin s:=mem[q].hh.rh;mem[q].hh.rh:=r;r:=q;
...
rr:=pp+1;while r<>30000 do begin ss:=getavail;mem[rr].hh.rh:=ss;rr:=ss;
mem[rr].hh.lh:=mem[r].hh.lh;r:=mem[r].hh.rh;end;mem[rr].hh.rh:=30000;
r:=mem[p+1].hh.lh;rr:=29999;while r>1 do begin ss:=getavail;
mem[rr].hh.rh:=ss;rr:=ss;mem[rr].hh.lh:=mem[r].hh.lh;r:=mem[r].hh.rh;
end;mem[rr].hh.rh:=r;mem[pp+1].hh.lh:=mem[29999].hh.rh{:335};end;
...
while p<>30000 do begin t:=mem[p].hh.lh-0;w:=t mod 8;
...
if q<>30000 then begin repeat mem[q].hh.lh:=8-2*((mem[q].hh.lh-0)mod 8)+
mem[q].hh.lh;q:=mem[q].hh.rh;until q=30000;{345:}u:=p+1;q:=mem[u].hh.rh;
r:=q;s:=mem[r].hh.rh;
while true do if mem[s].hh.lh>mem[r].hh.lh then begin mem[u].hh.rh:=q;
if s=30000 then goto 30;u:=r;q:=s;r:=q;s:=mem[r].hh.rh;
end else begin t:=s;s:=mem[t].hh.rh;mem[t].hh.rh:=q;q:=t;end;
30:mem[r].hh.rh:=30000{:345};end;p:=mem[p].hh.rh;end;mem[h+4].int:=0;
end;{:344}{346:}procedure sortedges(h:halfword);label 30;var k:halfword;
p,q,r,s:halfword;begin r:=mem[h+1].hh.lh;mem[h+1].hh.lh:=0;
p:=mem[r].hh.rh;mem[r].hh.rh:=30000;mem[29999].hh.rh:=r;
while p>1 do begin k:=mem[p].hh.lh;q:=29999;repeat r:=q;q:=mem[r].hh.rh;
until k<=mem[q].hh.lh;mem[r].hh.rh:=p;r:=mem[p].hh.rh;mem[p].hh.rh:=q;
p:=r;end;{347:}begin r:=h+1;q:=mem[r].hh.rh;p:=mem[29999].hh.rh;
...
if s=30000 then goto 30;r:=p;p:=s;end;30:end{:347};end;
...
if mem[p+1].hh.rh<>30000 then{349:}begin r:=29999;q:=mem[p+1].hh.rh;
ww:=0;m:=1000000;prevw:=0;
while true do begin if q=30000 then mm:=1000000 else begin d:=mem[q].hh.
...
if q=30000 then goto 30;end;m:=mm;
...
end;q:=s;end;30:mem[r].hh.rh:=30000;mem[p+1].hh.rh:=mem[29999].hh.rh;
if r<>29999 then{351:}begin if minn=65535 then minn:=n;maxn:=n;
if mind>mem[mem[29999].hh.rh].hh.lh then mind:=mem[mem[29999].hh.rh].hh.
...
for j:=0 to mspread do move[j]:=30000{:356};{355:}p:=getnode(2);
mem[p+1].hh.rh:=30000;mem[p+1].hh.lh:=0;mem[p].hh.lh:=curedges;
mem[mem[curedges].hh.rh].hh.lh:=p;p:=getnode(2);mem[p+1].hh.rh:=30000;
...
end else begin if r=30000 then goto 30;dw:=-((rd mod 8)-4);
...
freenode(p,2);{364:}move[mspread]:=0;j:=0;while move[j]=30000 do j:=j+1;
...
while move[jj]=30000 do jj:=jj-1;mem[curedges+1].hh.lh:=j+mm;
...
while qq<>30000 do begin mem[qq].hh.lh:=mem[qq].hh.lh+delta;
...
if qq<>30000 then begin if mem[p+1].hh.lh=1 then mem[p+1].hh.lh:=0;
mem[pp+1].hh.rh:=30000;r:=p+1;q:=mem[r].hh.rh;
if q=30000 then mem[p+1].hh.rh:=qq else while true do begin k:=mem[qq].
hh.lh;while k>mem[q].hh.lh do begin r:=q;q:=mem[r].hh.rh;end;
mem[r].hh.rh:=qq;rr:=mem[qq].hh.rh;mem[qq].hh.rh:=q;
if rr=30000 then goto 30;r:=qq;qq:=rr;end;end;30:{:368};
...
while q<>30000 do{370:}begin m:=mem[q].hh.lh-0;
...
k:1..8;h:halfword;m,n:integer;w,ww:halfword;begin p:=29999;
...
end;end;if p=29999 then begin w:=mem[penhead+1].hh.rh;
p:=trivialknot(mem[w+1].int+mem[w+2].int,mem[w+2].int);
mem[29999].hh.rh:=p;end;mem[p].hh.rh:=mem[29999].hh.rh;
makepath:=mem[29999].hh.rh;end;
...
while true do begin if q=30000 then d:=rightedge else d:=mem[q].hh.lh-0;
...
pwithxbecomingq:=pplusfq(mem[29999].hh.rh,v,q,t,17);end;end;
...
n:=mem[x+1].int mod 64;{612:}s:=29999;mem[s].hh.rh:=p;r:=p;
...
finalnode:=r;p:=mem[29999].hh.rh{:612};
...
r:=mem[prevr].hh.rh;end{:614};{615:}if n>0 then{616:}begin s:=29999;
...
r:=mem[s].hh.rh;until mem[s].hh.lh=0;p:=mem[29999].hh.rh;end{:616};
...
{636:}if(fileptr=inputptr)or(curinput.indexfield<=6)or(curinput.
indexfield<>10)or(curinput.locfield<>0)then begin tally:=0;
oldsetting:=selector;
if(curinput.indexfield<=6)then begin{637:}if curinput.namefield<=1 then
...
end else begin{638:}case curinput.indexfield of 7:printnl(607);
8:{639:}begin printnl(612);p:=paramstack[curinput.limitfield];
if p<>0 then if mem[p].hh.rh=1 then printexp(p,0)else showtokenlist(p,0,
20,tally);print(613);end{:639};9:printnl(608);
10:if curinput.locfield=0 then printnl(609)else printnl(610);
11:printnl(611);12:begin println;
...
begintokenlist(p,10);end;{:652}{653:}procedure backerror;
...
begin if inopen=6 then overflow(615,6);
...
mem[p].hh.lh:=cursym;begintokenlist(p,10);end{:662};
...
begin 20:cursym:=0;
if(curinput.indexfield<=6)then{669:}begin 25:c:=buffer[curinput.locfield
...
break(termout);forceeof:=false;endfilereading;
...
then printnl(651);println;first:=curinput.startfield;begin;print(42);
...
end else fatalerror(652);end{:679};
...
begintokenlist(paramstack[curinput.limitfield+cursym-(2520)],9);goto 20;
...
balance:=1;mem[29998].hh.rh:=0;while true do begin getnext;
...
end else begin begin if interaction=3 then;printnl(261);print(924);end;
...
30:curexp:=mem[29998].hh.rh;curtype:=20;scannerstatus:=0;end;
...
{736:}while(curinput.indexfield>6)and(curinput.locfield=0)do
endtokenlist;
...
5:{712:}begin while(curinput.indexfield>6)and(curinput.locfield=0)do
...
if curcmd<11 then expand else backinput;begintokenlist(p,10);end{:715};
...
end else begin begintokenlist(mem[loopptr].hh.lh,7);goto 10;end;
...
if interaction<2 then fatalerror(749);breakin(termin,true);begin;
print(750);terminput;end;{787:}begin beginname;k:=first;
...
begin oldsetting:=selector;if jobname=0 then jobname:=751;
packjobname(752);while not aopenout(logfile)do{789:}begin selector:=1;
promptfilename(754,752);end{:789};logname:=amakenamestring(logfile);
selector:=2;logopened:=true;
{790:}begin write(logfile,'This is METAFONT, Version 2.7182818');
slowprint(baseident);print(755);printint(roundunscaled(internal[16]));
...
printdd(m mod 60);end{:790};inputstack[inputptr]:=curinput;printnl(753);
...
begin{795:}while(curinput.indexfield>6)and(curinput.locfield=0)do
endtokenlist;
if(curinput.indexfield>6)then begin begin if interaction=3 then;
printnl(261);print(757);end;begin helpptr:=3;helpline[2]:=758;
helpline[1]:=759;helpline[0]:=760;end;error;end;
if(curinput.indexfield<=6)then scanfilename else begin curname:=285;
...
slowprint(curinput.namefield);break(termout);
if curinput.namefield=strptr-1 then begin flushstring(curinput.namefield
);curinput.namefield:=curname;end;{794:}begin line:=1;
...
begin begin if interaction=3 then;printnl(261);print(s);end;print(770);
...
begin begin if interaction=3 then;printnl(261);print(790);end;
...
print(806);end;begin helpptr:=2;helpline[1]:=807;helpline[0]:=575;end;
...
helpline[2]:=819;helpline[1]:=820;helpline[0]:=697;end;backerror;end;
...
337)else print(836)else print(837);printchar(41);end;
...
print(846);end;printint(n);printchar(41);begin helpptr:=1;
helpline[0]:=847;end;putgeterror;end{:914};end;flushcurexp(n*65536);end;
...
begin disperr(p,285);disperr(0,838);if c>=94 then printop(c);
...
printnl(261);print(867);end;begin helpptr:=3;helpline[2]:=868;
...
helpline[1]:=866;helpline[0]:=538;end;putgeterror;10:end;
...
begin for k:=a to b do write(gffile,gfbuf[k]);end;
...
{1164:}if internal[27]<=0 then gfext:=1054 else begin oldsetting:=
selector;selector:=5;printchar(46);
printint(makescaled(internal[27],59429463));print(1055);
...
selector:=5;print(1053);printint(roundunscaled(internal[14]));
...
if f<>0 then begin printchar(46);printint(f);end;break(termout);
...
repeat if q=30000 then mm:=268435456 else begin d:=mem[q].hh.lh-0;
...
until mm=268435456;if w<>0 then printnl(1057);
...
break(termout);if internal[11]>0 then printedges(1056,true,xoff,yoff);
end;{:1165}{995:}{1006:}procedure tryeq(l,r:halfword);label 30,31;
...
begin if interaction=3 then;printnl(261);print(897);end;print(899);
...
print(894);end;begin helpptr:=2;helpline[1]:=895;helpline[0]:=896;end;
putgeterror;goto 30;45:begin if interaction=3 then;printnl(261);
print(897);end;begin helpptr:=2;helpline[1]:=898;helpline[0]:=896;end;
...
{:1003}end;{1002:}disperr(lhs,285);disperr(0,891);
...
.lh-(2369)]:=curexp else begin disperr(0,887);
slowprint(intname[mem[lhs].hh.lh-(2369)]);print(888);begin helpptr:=2;
...
end else begin begin if interaction=3 then;printnl(261);print(900);end;
...
printnl(261);print(903);end;begin helpptr:=5;helpline[4]:=904;
helpline[3]:=905;helpline[2]:=906;helpline[1]:=907;helpline[0]:=908;end;
...
begin helpptr:=3;helpline[2]:=932;helpline[1]:=672;helpline[0]:=933;end;
...
printnl(765);printexp(0,2);flushcurexp(0);until curcmd<>82;end;
{:1040}{1041:}procedure disptoken;begin printnl(940);
...
begin printnl(950);{printint(varused);printchar(38);printint(dynused);
if false then}print(358);print(558);printint(himemmin-lomemmax-1);
print(951);println;printnl(952);printint(strptr-initstrptr);
...
printvariablename(p);if mem[p].hh.b0>22 then print(664);print(953);
...
print(954);end;if interaction<3 then begin helpptr:=0;
errorcount:=errorcount-1;end else begin helpptr:=1;helpline[0]:=955;end;
...
interaction=3 then;printnl(261);print(967);end;begin helpptr:=1;
helpline[0]:=965;end;putgetflusherror(0);end;end{:1056};
...
printnl(261);print(790);end;showtokenlist(t,0,1000,0);print(968);
printtype(mem[p].hh.b0);printchar(41);begin helpptr:=2;helpline[1]:=969;
helpline[0]:=970;end;putgeterror;end else curedges:=mem[p+1].int;
...
print(978);end;begin helpptr:=2;helpline[1]:=979;helpline[0]:=970;end;
...
then;printnl(261);print(1001);end;print(intname[m]);print(1002);
begin helpptr:=1;helpline[0]:=1003;end;putgeterror;
...
goto 10;45:begin if interaction=3 then;printnl(261);print(987);end;
...
print(990);end;begin helpptr:=1;helpline[0]:=991;end;putgeterror;
...
helpline[0]:=995;end;putgeterror;
...
printnl(261);print(1035);end;begin helpptr:=1;helpline[0]:=1036;end;
...
print(1035);end;begin helpptr:=1;helpline[0]:=1036;end;error;ll:=ll;
...
helpline[1]:=1038;helpline[0]:=309;end;putgetflusherror(0);end;
kern[nk]:=curexp;k:=0;while kern[k]<>curexp do k:=k+1;
if k=nk then begin if nk=maxkerns then overflow(1034,maxkerns);nk:=nk+1;
...
printnl(261);print(1023);end;begin helpptr:=1;helpline[0]:=1024;end;
backerror;ligkern[nl].b1:=0;ligkern[nl].b2:=0;ligkern[nl].b3:=0;
ligkern[nl].b0:=129;end;
if nl=ligtablesize then overflow(1025,ligtablesize);nl:=nl+1;
...
helpline[0]:=1039;end;backerror;end;exten[ne].b0:=getcode+0;
if curcmd<>82 then begin missingerr(44);begin helpptr:=1;
helpline[0]:=1039;end;backerror;end;exten[ne].b1:=getcode+0;
if curcmd<>82 then begin missingerr(44);begin helpptr:=1;
helpline[0]:=1039;end;backerror;end;exten[ne].b2:=getcode+0;
if curcmd<>82 then begin missingerr(44);begin helpptr:=1;
helpline[0]:=1039;end;backerror;end;exten[ne].b3:=getcode+0;ne:=ne+1;
...
(1008,maxfontdimen);while j>np do begin np:=np+1;param[np]:=0;end;
...
if internal[34]>=0 then if curtype<>m then{1178:}begin disperr(0,1060);
begin helpptr:=1;helpline[0]:=1061;end;putgeterror;
...
print(1071);print(jobname);printchar(32);
...
while not wopenout(basefile)do promptfilename(1072,742);printnl(1073);
slowprint(wmakenamestring(basefile));flushstring(strptr-1);printnl(285);
slowprint(baseident){:1200};{1190:}begin basefile^.int:=166307429;
...
printint(strptr);print(1068);printint(poolptr){:1192};{1194:}sortavail;
...
print(1069);printint(varused);printchar(38);printint(dynused){:1194};
...
printint(stcount);print(1070){:1196};{1198:}begin basefile^.int:=intptr;
put(basefile);end;
...
interaction=3 then;printnl(261);print(869);end;
...
then begin printnl(285);slowprint(curexp);break(termout);end;
if internal[34]>0 then{1179:}begin if outputfilename=0 then initgf;
gfstring(1062,curexp);end{:1179};
...
print(875);end;begin helpptr:=6;helpline[5]:=876;helpline[4]:=877;
helpline[3]:=878;helpline[2]:=872;helpline[1]:=873;helpline[0]:=874;end;
...
print(910);end;begin helpptr:=2;helpline[1]:=911;helpline[0]:=689;end;
...
break(termout);end;packbufferedname(13,1,0);
...
begin{1191:}x:=basefile^.int;if x<>166307429 then goto 6666;
begin get(basefile);x:=basefile^.int;end;if x<>0 then goto 6666;
...
begin get(basefile);x:=basefile^.int;end;if x<>6 then goto 6666{:1191};
{1193:}begin begin get(basefile);x:=basefile^.int;end;
...
x:=basefile^.int;end;
if(x<1022)or(x>29997)then goto 6666 else lomemmax:=x;end;
...
begin begin get(basefile);x:=basefile^.int;end;
if(x<lomemmax+1)or(x>29998)then goto 6666 else himemmin:=x;end;
begin begin get(basefile);x:=basefile^.int;end;
...
if(x<>69069)or eof(basefile)then goto 6666{:1199};loadbasefile:=true;
...
printnl(261);print(780);end;printint(groupline);print(781);
...
print(784);end;begin helpptr:=1;helpline[0]:=785;end;error;
...
if p<>0 then makeexpcopy(p)else begin obliterated(q);helpline[2]:=797;
helpline[1]:=798;helpline[0]:=799;putgetflusherror(0);end;
flushnodelist(q);goto 30{:852};end{:844};others:begin badexp(769);
...
helpline[2]:=801;helpline[1]:=802;helpline[0]:=697;end;backerror;end;
...
if curcmd<>64 then begin missingerr(93);begin helpptr:=3;
helpline[2]:=803;helpline[1]:=802;helpline[0]:=697;end;backerror;end;
...
print(828);end;begin helpptr:=3;helpline[2]:=829;helpline[1]:=830;
helpline[0]:=831;end;putgeterror;d:=47;mem[q+6].int:=65536;y:=65536;end;
...
mem[lomemmax].hh.lh:=0{:1207};{1124:}mem[29999].hh.rh:=19;
...
fixchecksum;if internal[33]>0 then begin{1126:}mem[29999].hh.rh:=19;
...
if perturbation>=4096 then tfmwarning(21);mem[29999].hh.rh:=19;
...
if perturbation>=4096 then tfmwarning(22);mem[29999].hh.rh:=19;
...
maxkerns:1,'k,256e,',maxfontdimen:1,'p)');end[:1141];}printnl(1047);
...
if gfptr>0 then writegf(0,gfptr-1){:1156};printnl(1063);
slowprint(outputfilename);print(558);printint(totalchars);print(1064);
if totalchars<>1 then printchar(115);print(1065);
printint(gfoffset+gfptr);print(1066);bclose(gffile);end{:1182};
end{:1206};if logopened then begin writeln(logfile);aclose(logfile);
selector:=selector-2;if selector=1 then begin printnl(1074);
...
while openparens>0 do begin print(1075);openparens:=openparens-1;end;
while condptr<>0 do begin printnl(1076);printcmdmod(2,curif);
if ifline<>0 then begin print(1077);printint(ifline);end;print(1078);
...
then begin selector:=1;printnl(1079);selector:=3;end;
if c=1 then begin storebasefile;goto 10;printnl(1080);goto 10;end;
...
{:1108}{1176:}primitive(1058,29,4);primitive(1059,29,16);{:1176};end;
...
for k:=29998 to 30000 do mem[k]:=mem[lomemmax];avail:=0;memend:=30000;
himemmin:=29998;varused:=23;dynused:=30001-himemmin;
...
mem[19].hh.rh:=0;{:229}{324:}mem[30000].hh.lh:=65535;
...
{:1127}{1185:}baseident:=1067;{:1185}end;
{:1210}{1212:}{procedure debughelp;label 888,10;var k,l,m,n:integer;
begin while true do begin;printnl(1081);break(termout);read(termin,m);
if m<0 then goto 10 else if m=0 then begin goto 888;
888:m:=0;
['BREAKPOINT']
end else begin read(termin,n);case m of[1213:]1:printword(mem[n]);
...
{:1212}{:1202}{1204:}begin history:=3;rewrite(termout,'TTY:','/O');
...
{:14}{154:}if memmax<>30000 then bad:=10;if memmax<30000 then bad:=10;
...
{:777}if bad>0 then begin writeln(termout,
...
fileoffset:=0;
{:55}{61:}write(termout,'This is METAFONT, Version 2.7182818');
if baseident=0 then writeln(termout,' (no base preloaded)')else begin
slowprint(baseident);println;end;break(termout);{:61}{783:}jobname:=0;
